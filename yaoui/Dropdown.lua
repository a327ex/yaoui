...
function Dropdown:new(yui, settings)
    ...
    self.font = love.graphics.newFont(self.yui.Theme.open_sans_bold, math.floor(self.size*0.7))
    ...
    for i, option in ipairs(self.options) do
        self.down_area:addElement(self.yui.UI.Button(0, self.size/2 + (i-1)*h, math.max(min_w, self.w), h, {
            ...
            font = love.graphics.newFont(self.yui.Theme.open_sans_bold, math.floor(self.size*0.65)),
            text = self.options[i],
            size = self.size,
        }))
    end
